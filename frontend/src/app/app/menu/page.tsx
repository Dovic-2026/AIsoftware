  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end animate-fade-in" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white w-full rounded-t-[28px] max-h-[90vh] overflow-y-auto">
        <div className="w-10 h-1 bg-gray-200 rounded-full mx-auto mt-3 mb-4 sticky top-3" />
  ...
  const { mutate: createItem, isPending } = useMutation({
    ...
    onSuccess: async (res: any) => {
      ...
      if (imageFile) {
        ...
        try {
          await menuApi.uploadImage(restaurant!.id, newId, imageFile);
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white px-5 pt-4 pb-4 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => { setShowItemSheet(true); setImageFile(null); setImagePreview(null); reset(); }}
            className="bg-gradient-to-r from-[#25D366] to-[#128C7E] text-white px-4 py-2 rounded-xl text-[13px] font-bold shadow-sm">
            + Add Item
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto hide-scroll px-4 py-3">
        {loadItems ? (
          <div className="grid grid-cols-2 gap-3">{Array(6).fill(0).map((_, i) => <div key={i} className="h-48 rounded-2xl shimmer" />)}</div>
      ...
      <Sheet open={showItemSheet} onClose={() => setShowItemSheet(false)} title="Add Menu Item">
        <form onSubmit={handleSubmit((d) => createItem(d))} className="px-5 py-4 space-y-4">
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              ...
              {imagePreview ? (
                ...
                <>
                  ...
                  <span className="text-xs text-gray-400 font-semibold">Tap to upload photo</span>
                </>
              )}
            </button>
          ...
          <div className="flex gap-3 pb-2">
            ...
            <button type="submit" disabled={isPending || uploadingImg} className="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-[#25D366] to-[#128C7E] text-white text-[14px] font-bold flex items-center justify-center gap-2">
              {(isPending || uploadingImg) ? <Spinner /> : "Add Item"}
            </button>
          </div>
        </form>
      </Sheet>
